<script lang="ts">
  ...
  async function loadOverview() {
    ...
    error = false;
    try {
      overview = await getOverview(me.communityKey, period);
    } catch {
      error = true;
    } finally {
      loading = false;
  ...
  {:else}
    ...
    <section class="dashboard-grid">
      ...
      <article class="panel meters-panel">
        <header><h2>{$_('overview.meters')}</h2><a href="/data-flow">Apri dettaglio →</a></header>
      ...
      <article class="panel window-panel">
        ...
        <a class="detail-link" href="/flexibility">{$_('overview.view_detail')} →</a>
      </article>
    </section>
  {/if}
</div>
